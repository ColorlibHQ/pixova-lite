...
# Themes that keep their rules somewhere other than the root style.css pass the
# extra stylesheets to scan for content:"\fXXX" icons as further arguments.
extra_css = [pathlib.Path(a) for a in sys.argv[4:]]
...
for f in list(theme_dir.rglob('*.php')) + list(theme_dir.rglob('*.js')) + list(theme_dir.rglob('*.json')):
    sp = str(f)
    # 'assets' is where these themes keep the icon *catalogue* the Customizer
    # picker offers -- every name Font Awesome has. Subsetting from that would
    # subset nothing away, so only usage is scanned, never the catalogue.
    if any(k in sp for k in ('/.git/','node_modules','fontawesome','tgmpa','/assets/')): continue
    ...
    # Written-out style classes, wherever they appear: a Customizer default, a
    # demo-content value, a class attribute inside an escaped JS string. The
    # scan above only sees class="..." with unescaped quotes.
    for style, name in re.findall(r'\bfa-(solid|regular|brands)\s+fa-([a-z0-9-]+)', t):
        if style == 'brands':
            if name in brand_map: used_brands.add(brand_map[name])
        elif name in core_map:
            (used_regular if style == 'regular' else used_solid).add(core_map[name])
...
for cssf in [theme_dir/'style.css', theme_dir/base/'css'/'custom.css', theme_dir/'rtl.css'] + extra_css:
...
# A manifest of what the subset can actually draw. Themes that let a user pick
# an icon in the Customizer read this at enqueue time: if the configured icon is
# not in here the subset would render an empty box, so they load the full family
# instead. A PHP array rather than JSON so opcache keeps it in memory.
manifest = []
for style, cps, mp in (('fa-solid', used_solid, core_map), ('fa-regular', used_regular, core_map),
                       ('fa-brands', used_brands, brand_map)):
    for name, cp in sorted(mp.items()):
        if cp in cps: manifest.append(f'{style} fa-{name}')
(out / 'icons.php').write_text(
    "<?php\n/**\n * Icon classes present in fontawesome-subset.min.css.\n *\n"
    " * Generated by .github/build-icon-subset.py -- do not edit by hand.\n"
    " *\n * @package %s\n */\n\nreturn array(\n%s);\n"
    % (prefix, ''.join("\t'%s',\n" % c for c in sorted(manifest))))
