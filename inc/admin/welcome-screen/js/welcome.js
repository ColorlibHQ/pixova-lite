jQuery(document).ready(function() {
    ...
    if ( (typeof pixova_nr_actions_required !== 'undefined') && (pixova_nr_actions_required != '0') ) {
        jQuery('li.pixova-lite-w-red-tab a').append('<span class="pixova-lite-actions-count">' + pixova_nr_actions_required + '</span>');
    }
    ...
    jQuery(".pixova-dismiss-required-action").click(function(){
        ...
        jQuery.ajax({
            ...
            beforeSend : function(data,settings){
				jQuery('.pixova-lite-tab-pane#actions_required h1').append('<div id="temp_load" style="text-align:center"><img src="' + pixovaLiteWelcomeScreenObject.template_directory + '/inc/admin/welcome-screen/img/ajax-loader.gif" /></div>');
            },
            success    : function(data){
				jQuery("#temp_load").remove(); /* Remove loading gif */
                jQuery('#'+ data).parent().remove(); /* Remove required action box */

                var pixova_lite_actions_count = jQuery('.pixova-lite-actions-count').text(); /* Decrease or remove the counter for required actions */
                if( typeof pixova_lite_actions_count !== 'undefined' ) {
                    if( pixova_lite_actions_count == '1' ) {
                        jQuery('.pixova-lite-actions-count').remove();
                        jQuery('.pixova-lite-tab-pane#actions_required').append('<p>' + pixovaLiteWelcomeScreenObject.no_required_actions_text + '</p>');
                    }
                    else {
                        jQuery('.pixova-lite-actions-count').text(parseInt(pixova_lite_actions_count) - 1);
                    }
                }
            },
	...
	function pixova_welcome_page_tabs(event) {
        ...
        jQuery(".pixova-lite-tab-pane").not(tab).css("display", "none");
        jQuery(tab).fadeIn();
	}
    ...
    jQuery(".pixova-lite-nav-tabs a").click(function(event) {
        event.preventDefault();
		pixova_welcome_page_tabs(this);
    });

		/* Tab Content height matches admin menu height for scrolling purpouses */
	 $tab = jQuery('.pixova-lite-tab-content > div');
